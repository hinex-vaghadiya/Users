...
def trigger_backup(request):
    ...
    try:
        # 2. Run dumpdata for desired apps (we only need 'accounts' for this project as it holds users)
        output = StringIO()
        call_command(
            "dumpdata",
            "accounts",  # Backing up the main app which has the custom Accounts model
            "--indent",
            "2",
            "--natural-foreign",
            "--natural-primary",
